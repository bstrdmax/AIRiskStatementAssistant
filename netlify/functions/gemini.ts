...
export const handler: Handler = async (event) => {
    ...
    try {
        ...
        if (action === 'fiveWhys') {
            const { concern } = body;
            if (!concern) {
                return { statusCode: 400, body: JSON.stringify({ error: "Missing 'concern' in request body." }) };
            }
        ...
        } else if (action === 'generateDetails') {
            const { rootCause, originalConcern } = body;
            if (!rootCause || !originalConcern) {
                return { statusCode: 400, body: JSON.stringify({ error: "Missing 'rootCause' or 'originalConcern' in request body." }) };
            }
